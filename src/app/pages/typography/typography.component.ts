...
export class TypographyComponent implements OnInit {
  ...
  dateDebut;
   ...
   handleForExcell(arg){
    ...
    for(let i of arg){
        b.push({date:i.date_commande,
          id_transaction:i.id,
          nomservice:i.service,
          traitement:i.Paiement,
          numero:i.numero,
          montant:this.currencyFormat(i.montant)})
    }
    return b;
}
  exportToExcel(): void {
...
errorMessage:number = 0
  ...
  recherche(){
    this._serviceAdmin.historiqueMobileBulk({debut:this.dateDebut+" 00:00:00",fin:this.dateFin+" 23:59:59",token:JSON.parse(localStorage.getItem("currentuser")).token}).then(res=>{
      //console.log(res)
      if(res['status'] == 1){
  ...
  listTemp
   ngOnInit() {
     ...
     this._serviceAdmin.getsoldeMobileBulk({token:JSON.parse(localStorage.getItem("currentuser")).token}).then(res=>{
       //console.log(res)
       if(res['status'] == 1){
